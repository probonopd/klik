...
class KlikCreate (object):
	...
	def create(self, recipe, location):
		...
		for package in recipe.packages:
			...
			try:		
				...
				if package.md5:
					...
					md5_hash = md5.new(md5_file.read()).hexdigest()
					md5_file.close()
					
					if package.md5 != md5_hash:
				...
				raise ExecuteException( _("Error occured while attempting to download %s") % package.href )
		...
		if recipe.icon == None:
			...
			if recipe.icon[0] == "/":
				relative_path = os.path.join(root_directory, recipe.icon[1:]) 
		...
		return self.pack(root_directory, app_path, build_path=build_path)
