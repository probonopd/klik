...
{
  const char *fakechroot_env = getenv("FAKECHROOT_BASE");
  const int fakechroot_length = fakechroot_env ? strlen(fakechroot_env) : 0;
  ...
  if (fakechroot_env)
    buf = alloca (strlen(fakechroot_env) + max_dirnamelen + max_capstrlen + namelen +1);
  else buf = alloca (max_dirnamelen + max_capstrlen + namelen);
  do
    {
	  ...
	  && current_what != this_dir->what)
	{
      ...
      if (fakechroot_env)
        {
          if (memcmp(this_dir->dirname,fakechroot_env,fakechroot_length) != 0)
            {
            __mempcpy (buf, fakechroot_env, fakechroot_length);
            edp = (char *) __mempcpy (&(buf[fakechroot_length]), this_dir->dirname, this_dir->dirnamelen);
            }
          else edp = (char *) __mempcpy (buf, this_dir->dirname, this_dir->dirnamelen);
        }
      else edp = (char *) __mempcpy (buf, this_dir->dirname, this_dir->dirnamelen);
		...
		int type, int trace_mode, int mode, Lmid_t nsid)
{
  ...
  if (strchr (name, '/') == NULL)
	  ...
	  if (cached != NULL)
		  ...
		  __builtin_expect (l->l_flags_1 & DF_1_NODEFLIB, 0))
		{
		  ...
		  do
			...
			{
                          if (fakechroot_env && cached!=NULL) free((void*)cached);
    ...
    if (realname==NULL && fakechroot_env)
      {
        char *new_name = malloc(strlen(name) + fakechroot_length + 2);
        strcpy(new_name, fakechroot_env);
        strcat(new_name, "/");
        strcat(new_name, name);

        // The path may contain dynamic string tokens.
        realname = (loader
                  ? expand_dynamic_string_token (loader, new_name)
                  : new_name);
