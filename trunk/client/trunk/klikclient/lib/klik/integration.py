...
class KlikIntegration(object):
	...
	def notify_event(self, message, applications, notify_icon="application-x-application-cmg"):
		if applications and self.base.settings.application_folder_notify:
			...
			if n_apps > 1:
				notify_icon = "application-x-application-cmg"
	...
	def __is_valid_cmg_name(self, path):
		...
		return KlikCmg(self.base,path).is_valid_cmg() or path.lower().endswith( ".cmg" )
	...
	def sync(self, apps_dirs):
		...
		registered_cmgs = filter( lambda x: KlikCmg(self.base,x).is_valid_cmg, registered_cmgs ) # list of valid CMGs
