...
class XdgUtils (object):
	...
	def __init__(self, xdg_path = None):
		...
		if self.path == None or self.path == "":
			self.path = os.path.abspath(os.path.join(sys.path[0], os.path.pardir, "lib" , "klik", "utils", "xdg-utils"))
	...
	def execute_su(self, command, args=None):
		xdg_su = self.__get_command("xdg-su")
		command =  ["sh", xdg_su, "-c"] + [command]
		if args:
	...
	def get_installed_libs(self):
		...
		if self.__installed_libs == None:
			result = []
			result.append("PYTHON " + platform.python_version())

			# GNOME
			if len(Popen(["which", "gnome-about"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0].strip()) > 0:
				cmd_result = Popen(["gnome-about", "--version"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0]
				if cmd_result:
					version = re.search("\d+(\.\d+)+", cmd_result.strip()).group()
					result.append("GNOME " + version)
			# GTK
			if self.get_is_gtk_installed():
				import gtk
				maj, min, rev = gtk.gtk_version
				result.append("GTK %d.%d.%d" % (maj, min, rev) )
			...
			if len(Popen(["which", "kde-config"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0].strip()) > 0:
				try:
					if Popen(["kde-config" ,"--version"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0].strip().count("KDE") > 0:
						text = Popen(["kde-config" ,"--version"], stdout=PIPE, stderr=STDOUT).communicate()[0].strip()
			...
			if len(Popen(["which", "mono"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0].strip()) > 0:
				version = re.search("\d+(\.\d+)+", Popen(["mono", "--version"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0].strip()).group()
				result.append("MONO " + version)

			# Java
			if len(Popen(["which", "java"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0].strip()) > 0:
				text = Popen(["java", "-version"], stdout=PIPE, stderr=STDOUT).communicate()[0].strip()
			...
			if len(Popen(["which", "wine"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0].strip()) > 0:
				version = re.search("\d+(\.\d+)+", Popen(["wine", "--version"], stdout=PIPE, stderr=STDOUT).communicate()[0].strip()).group()
				result.append("WINE " + version)

			# Video Acclerated
			if len(Popen(["which", "glxinfo"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0].strip()) > 0:
				glxinfo = Popen(["glxinfo"], stdout=PIPE, stderr=open(os.devnull, "w")).communicate()[0].strip()
				direct_rendering = re.search("(?<=direct rendering: ).*", glxinfo)
				if direct_rendering:
					result.append("DIRECT_RENDERING " + direct_rendering.group().upper())
...
if __name__ == "__main__":
	xdg = XdgUtils("/home/jason/code/klik/trunk/client/trunk/klikclient/lib/klik/utils/xdg-utils")
	xdg.execute_su("gedit")
