# IMPORTED FROM zero-install/0launch/zeroinstall/injector
...
class ValidSig(Signature):
	...
	def get_details(self):
		cout = Popen(('gpg', '--with-colons', '--no-secmem-warning', '--list-keys', self.fingerprint),
...
def load_keys(fingerprints):
	...
	cout = Popen(['gpg', '--fixed-list-mode', '--with-colons', '--list-keys', '--with-fingerprint'] + fingerprints,
			stdout=PIPE, close_fds=True).stdout
	try:
		for line in cout:
...
def _check_xml_stream(stream):
	...
	return _check_xml_string(data_to_check)
